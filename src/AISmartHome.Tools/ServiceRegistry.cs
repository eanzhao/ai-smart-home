...
public class ServiceRegistry
{
    ...
    private List<ServiceDomain> _serviceDomains = new();
    private Dictionary<string, ServiceDefinition> _serviceIndex = new(); // Key: "domain.service"
    ...
    {
        var servicesResult = await _client.Services.GetAsync(cancellationToken: ct);
        var services = servicesResult ?? new List<Services>();
        
        // Convert to simplified model
        _serviceDomains = services.Select(s => s.ToServiceDomain()).ToList();
    ...
    /// </summary>
    public async Task<List<ServiceDomain>> GetAllDomainsAsync(CancellationToken ct = default)
    {
    ...
    /// </summary>
    public async Task<ServiceDefinition?> GetServiceAsync(string domain, string service, CancellationToken ct = default)
    {
        await EnsureFreshAsync(ct);
    ...
    /// </summary>
    public async Task<Dictionary<string, ServiceDefinition>> GetDomainServicesAsync(string domain, CancellationToken ct = default)
    {
        await EnsureFreshAsync(ct);
        ...
        return domainData?.Services ?? new Dictionary<string, ServiceDefinition>();
    }
